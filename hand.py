...
# ESP32 연결
esp32 = serial.Serial(port='COM3', baudrate=9600, timeout=1)  # ESP32 포트를 설정하세요
        ...
        if result.multi_hand_landmarks:
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                for i in range(len(FINGER_TIPS)):
                    ...
                    distances.append(int(distance))  # 정수로 변환
                ...
                # ESP32로 거리 데이터 전송
                data = ','.join(map(str, distances))  # "거리1,거리2,..." 형식으로 데이터 생성aa
                esp32.write(f'{data}\n'.encode())  # ESP32로 데이터 전송
